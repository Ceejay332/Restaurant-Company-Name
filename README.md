# Restaurant Menu Webpage

This project is a simple, responsive HTML and CSS template for displaying a restaurant or company menu. It provides a clean, visually appealing layout with sections for different meal types, opening hours, and contact links. The template is fully customizable for your own brand, menu items, and styling.

## Live Preview
Check out the live version of this project:[view live menu](https://ceejay332.github.io/Restaurant-Company-Name/)

## Features

## • Responsive Layout: 
The menu is centered and scales nicely on different screen sizes.

## • Menu Sections: 
Separate sections for breakfast, lunch, dinner, or other categories.

## • Item Display: 
Each menu item has a dish name and price.

## • Images:
Option to include images for each section.

## • Opening Hours:
Simple section for business hours.

## • Contact Links: 
Direct links for WhatsApp, phone calls, and location.

## • Custom Styling:
Easy-to-edit CSS for colors, fonts, and layout.

## File Structure

project-folder/

│

├─ index.html         # Main HTML file

├─ styles.css         # CSS styling file

├─ images/            # Folder for images (menu images, background)

│   ├─ breakfast.jpg

│   ├─ lunch2.jpg

│   └─ cookshop.jpg

## Usage

1. Clone or download the repository.

2. Replace placeholder text:

• Restaurant/Company Name → Your restaurant or company name

• Est Date → Year of establishment

• Menu Title → Name of the menu section (e.g., Breakfast, Lunch)

• Item and Price → Your menu items and their prices

3. Replace images in the images folder with your own.

4. Customize styles in styles.css as needed (colors, fonts, backgrounds).

5. Open index.html in a browser to view your menu.

## Styling Notes

• Background image is applied to the whole page.

• Menu container has a distinct color and is centered with a max-width.

• Menu items are aligned with dish names on the left and prices on the right.

• Header and section titles use Impact font for a bold, professional look.

• Footer contains contact links with hover effects.

## Contact Links

• WhatsApp: Clickable link to send a message via WhatsApp.

• Phone: Clickable link to call the restaurant.

• Location: Link to a map or location page.

## Customization Tips

• Add more menu sections by duplicating <section> blocks.

• Adjust font sizes and colors in styles.css to match your branding.

• Add or remove menu items by editing the <article class="item"> elements.

• Update img elements to show your own food images.

This template is ideal for restaurants, cafés, or small businesses looking for a simple web menu without complex frameworks.
